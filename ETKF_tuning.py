...
#############################################################
### 本体 ###
def ETKF(X_forcast, m, H, y, R, inf, add):
    # 偏差の生成
...
###############################################################
def run(list):
    x_true, X_start, m, inf, add, r, eta = list
    # 初期値
    X_analise = X_start
    H, R = make_all()
    R = r*R
    k = 0
    file = open("./ETKF_tuning/time_series_add={:.1f}_inf={:.2f}_R={:.1f}.csv".format(add,inf,r), 'w')
    ...
    try:
        ...
        for i in range(1460):
            ...
            error = make_add_error(X_forcast, eta, x_true[k,:], add, k, H) # 相関のある誤差
            ...
            X_analise,B_tmp,A_tmp, ob_ob_tmp,oa_oa_tmp,ab_ab_tmp,ab_ob_tmp,oa_ob_tmp,ab_oa_tmp,HBH_tmp,HAH_tmp \
                = ETKF(X_forcast, m, H, y, R, inf, add)
        ...
        HBH = 0
        HAH = 0
        print("Error add={:.1f} R={:.1f} inf={:.2f}".format(add,r,inf))
    else:
        ...
        HBH = np.mean(HBH)
        HAH = np.mean(HAH)
        print("Clear :: add_true={:.1f} R={:.1f} inf={:.2f}".format(add,r,inf))
    file.close()
    return add, inf, r, a_error, o_error, f_error, correlation, B,A, ob_ob,oa_oa,ab_ab,ab_ob,oa_ob,ab_oa,HBH,HAH
...
def output_all(m):
    ...
    set_add = np.array([-1.0 + 0.1*i for i in range(20)])
    add_size = set_add.shape[0]
    ...
    set_r = np.array([1.0 + 0.1*i for i in range(10)])
    set_r = np.hstack((set_r, [2.0 + 1.0*i for i in range(9)]))
    set_r = np.hstack((set_r, [11.0 + 1.0*i for i in range(10)]))
    r_size = set_r.shape[0]
    ...
    for i in range(add_size):
        for j in range(inf_size):
            for k in range(r_size):
                tmp_list = [x_true, X_start, m, set_inf[j], set_add[i], set_r[k], eta]
                list.append(tmp_list)

    os.makedirs("./ETKF_tuning", exist_ok=True)
    ...
    ## 出力　##
    # CSV出力
    file = open("ETKF_tuning.csv", 'w')
    file.writelines("#ens={:d} error=0\n".format(m))
    file.writelines("add_true,multi_inf,r,analysis_error,observation_error,forecast_error,correlation,spread_B,spread_A,ob_ob,oa_oa,ab_ab,ab_ob,oa_ob,ab_oa,HBH,HAH\n")
