...
###############################################################
def run(list):
    x_true, X_start, m, inf, add_true, add_sys, r, eta, file_name = list
    # 初期値
    X_analise = X_start
    H, R = make_all()
    R = r*R
    k = 0
    file = open(file_name+"/time_series_add={:.1f}_inf={:.2f}.csv".format(add_true,inf), 'w')
    ...
    try:
        ...
        HBH = 0
        HAH = 0
        print("Error add_true={:.1f} add_sys={:.2f} Ruc={:.1f} inf={:.2f}".format(add_true,add_sys,r,inf))
    else:
        ...
        HBH = np.mean(HBH)
        HAH = np.mean(HAH)
        print("Clear :: add_true={:.1f} add_sys={:.2f} Ruc={:.1f} inf={:.2f}".format(add_true,add_sys,r,inf))
    file.close()
    return add_true, add_sys, inf, r, a_error, o_error, f_error, correlation, B,A, ob_ob,oa_oa,ab_ab,ab_ob,oa_ob,ab_oa,HBH,HAH
...
def output_all(m):
    ...
    data = np.loadtxt("./estimated_parameters.csv", delimiter=',', skiprows=1)
    add_true = data[:,0]
    ...
    file_name = ["./abob_obob_ETKFCC","./aboa_obob_ETKFCC","./aboa_oaob_ETKFCC"]
    ...
    for k in range(3):
        ...
        for i in range(add_size):
            for j in range(inf_size):
                tmp_list = [x_true, X_start, m, set_inf[j], add_true[i],add_est[i], Ruc_est[i], eta, file_name[k]]
        ...
        file.writelines("add_true,add_sys,multi_inf,Ruc_sys,analysis_error,observation_error,forcast_error,correlation,spread_B,spread_A,ob_ob,oa_oa,ab_ab,ab_ob,oa_ob,ab_oa,HBH,HAH\n")
        np.savetxt(file, result, delimiter=',')
        file.close()

    print("End Estimated ETKFCC")
    print("")
