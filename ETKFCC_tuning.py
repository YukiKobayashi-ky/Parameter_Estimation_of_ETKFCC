...
def run(list):
    ...
    R = r*R
    k = 0
    file = open("./ETKFCC_tuning/time_series_add={:.1f}_inf={:.2f}.csv".format(add,inf), 'w')
    ...
    try:
        ...
        for i in range(1460):
            ...
            X_analise,B_tmp,A_tmp, ob_ob_tmp,oa_oa_tmp,ab_ab_tmp,ab_ob_tmp,oa_ob_tmp,ab_oa_tmp,HBH_tmp,HAH_tmp \
                = ETKFCC(X_forcast, m, H, y, R, inf, add)
        ...
        HBH = 0
        HAH = 0
        print("Error add={:.1f} Ruc={:.1f} inf={:.2f}".format(add,r,inf))
    else:
        ...
        HBH = np.mean(HBH)
        HAH = np.mean(HAH)
        print("Clear :: add_true={:.1f} Ruc={:.1f} inf={:.2f}".format(add,r,inf))
    file.close()
...
def output_all(m):
    ...
    for i in range(add_size):
        for j in range(inf_size):
            tmp_list = [x_true, X_start, m, set_inf[j], set_add[i], 1.0, eta]
            list.append(tmp_list)

    os.makedirs("./ETKFCC_tuning", exist_ok=True)
    ...
    ## 出力　##
    # CSV出力
    file = open("ETKFCC_tuning.csv", 'w')
    file.writelines("#ens={:d} error=0\n".format(m))
    file.writelines("add_true,multi_inf,Ruc,analysis_error,observation_error,forcast_error,correlation,spread_B,spread_A,ob_ob,oa_oa,ab_ab,ab_ob,oa_ob,ab_oa,HBH,HAH\n")
